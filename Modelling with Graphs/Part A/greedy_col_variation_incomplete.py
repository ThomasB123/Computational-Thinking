...
def find_next_vertex(G):
    ...
    '''
    '''
    must visit nodes in such an order that
    each new node visited is the neighbour of
    an already visited node and is also the smallest
    available one.
    Therefore count up from 1 and check
    each time if that node has any neighbours which 
